...
if nonideal_liq:
    ...
    V = [st.sidebar.number_input(f"V{i+1}", value=1.0) for i in range(n)]
...
if nonideal_gas:
    ...
    for i in range(n):
        for j in range(i, n):
            val = st.sidebar.number_input(f"B{i+1}{j+1}", value=0.0)
            Bik[i][j] = Bik[j][i] = val

A = [st.sidebar.number_input(f"A{i+1}", value=8.07131) for i in range(n)]
B = [st.sidebar.number_input(f"B{i+1}", value=1730.63) for i in range(n)]
C = [st.sidebar.number_input(f"C{i+1}", value=233.426) for i in range(n)]

Ptot = st.sidebar.number_input("System Pressure", value=101.325)
R = st.sidebar.number_input("Gas Constant R", value=8.314)
...
x = [st.sidebar.number_input(f"x{i+1}", value=1/n) for i in range(n)]

tol = st.sidebar.number_input("Temperature tolerance (%)", value=0.01)
